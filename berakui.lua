...
if getgenv then
	local ok, result = pcall(function() return getgenv().DISABLE_BerakUI_REQUESTS end)
	if ok and result then requestsDisabled = true end
	local ok2, result2 = pcall(function() return getgenv().BerakUI_ASSET_ID end)
	if ok2 and type(result2) == "number" then customAssetId = result2 end
	local ok3, result3 = pcall(function() return getgenv().BerakUI_SECURE end)
...
local function secureNotify(wType, title, content)
	...
	task.spawn(function()
		while not BerakUILibrary or not BerakUILibrary.Notify do task.wait(0.5) end
		BerakUILibrary:Notify({
			Title = title,
			Content = content,
			Duration = 8,
		})
...
local BerakUIFolder = "BerakUI"
local ConfigurationFolder = BerakUIFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		BerakUIOpen = {Type = 'bind', Value = 'K', Name = 'BerakUI Keybind'},
		-- buildwarnings
		-- BerakUIprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.BerakUIOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("BerakUI | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		if callSafely(isfolder, BerakUIFolder) then
			if callSafely(isfile, BerakUIFolder..'/settings'..ConfigurationExtension) then
				file = callSafely(readfile, BerakUIFolder..'/settings'..ConfigurationExtension)
			end
		end
		...
		if useStudio then
			file = [[
	{"General":{"BerakUIOpen":{"Value":"K","Type":"bind","Name":"BerakUI Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"BerakUI Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
		end
			...
			for categoryName, categoryTable in file do
				for settingName, setting in categoryTable do
					...
					if not (settingType == typeof(setting.Value)) then
						warn("BerakUI | Error parsing settings file. '"..settingName.."' must be a "..settingType)
						continue
					end
	...
	if not success then 
		if writefile then
			warn('BerakUI had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled and not useStudio then
	local fetchSuccess, fetchResult = pcall((game :: any).HttpGet, game, "https://raw.githubusercontent.com/SiriusSoftwareLtd/BerakUI/refs/heads/main/reporter.lua")
		...
		if execSuccess and Analytics then
			pcall(function()
				reporter = Analytics.new({
					url          = "https://BerakUI-collect.sirius-software-ltd.workers.dev",
					token        = ANALYTICS_TOKEN,
					product_name = "BerakUI",
					category     = "UILibrary",
				})
			end)
		end
...
local BerakUIAssetId = customAssetId or 10804731440
local BerakUI = useStudio and script.Parent:FindFirstChild('BerakUI') or game:GetObjects("rbxassetid://"..BerakUIAssetId)[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local BerakUIDestroyed = false -- True when BerakUILibrary:Destroy() is called

repeat
	if BerakUI:FindFirstChild('Build') and BerakUI.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('BerakUI | Build Mismatch')
		print('BerakUI may encounter issues as you are running an incompatible interface version ('.. ((BerakUI:FindFirstChild('Build') and BerakUI.Build.Value) or 'No Build') ..').\n\nThis version of BerakUI is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	local toDestroy
	toDestroy, BerakUI = BerakUI, useStudio and script.Parent:FindFirstChild('BerakUI') or game:GetObjects("rbxassetid://"..BerakUIAssetId)[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

BerakUI.Enabled = false

if gethui then
	BerakUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(BerakUI)
	BerakUI.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	BerakUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == BerakUI.Name and Interface ~= BerakUI then
			Interface.Enabled = false
			Interface.Name = "BerakUI-Old"
		end
	end
elseif not useStudio then
...
if secureMode and not customAssetId then
	secureNotify("default_asset", "Secure Mode", "You are using the default BerakUI asset ID. Set BerakUI_ASSET_ID to a custom upload to avoid detection.")
end

do
	local AssetPath = BerakUIFolder.."/Assets"
	local AssetBaseURL = "https://github.com/SiriusSoftwareLtd/BerakUI/blob/main/assets/"
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("BerakUI | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)

		if not ok then
			warn("BerakUI | Failed to load custom assets: "..tostring(err))
			secureNotify("asset_load_fail", "BerakUI", "Failed to load custom assets. UI images may not display correctly.")
		end
	else
		secureNotify("no_getcustomasset", "BerakUI", "Your executor does not support getcustomasset. Some UI images may not render correctly.")
	end


	BerakUI.Main.Shadow.Image.Image = customAssets[tostring(5587865193)]
	BerakUI.Main.Topbar.Hide.Image = customAssets[tostring(10137832201)]
	BerakUI.Main.Topbar.ChangeSize.Image = customAssets[tostring(10137941941)]
	BerakUI.Main.Topbar.Settings.Image = customAssets[tostring(80503127983237)]
	BerakUI.Main.Topbar.Icon.Image = customAssets[tostring(78137979054938)]
	BerakUI.Main.Topbar.Search.Image = customAssets["IconMagnifyingGlass2"]
	BerakUI.Main.Topbar.Search.ImageRectOffset = Vector2.new(0, 0)
	BerakUI.Main.Topbar.Search.ImageRectSize = Vector2.new(0, 0)
	BerakUI.Main.Elements.Template.Toggle.Switch.Shadow.Image = customAssets[tostring(3602733521)]
	BerakUI.Main.Elements.Template.Slider.Main.Shadow.Image = customAssets[tostring(3602733521)]
	BerakUI.Main.Elements.Template.Dropdown.Toggle.Image = customAssets["IconChevronTopMedium"]
	BerakUI.Main.Elements.Template.Dropdown.Toggle.ImageRectOffset = Vector2.new(0, 0)
	BerakUI.Main.Elements.Template.Dropdown.Toggle.ImageRectSize = Vector2.new(0, 0)
	BerakUI.Main.Elements.Template.Label.Icon.Image = customAssets[tostring(11745872910)]
	BerakUI.Main.Elements.Template.ColorPicker.CPBackground.MainCP.Image = customAssets[tostring(11413591840)]
	BerakUI.Main.Elements.Template.ColorPicker.CPBackground.MainCP.MainPoint.Image = customAssets[tostring(3259050989)]
	BerakUI.Main.Elements.Template.ColorPicker.ColorSlider.SliderPoint.Image = customAssets[tostring(3259050989)]
	BerakUI.Main.TabList.Template.Image.Image = customAssets[tostring(4483362458)]
	BerakUI.Main.Search.Search.Image = customAssets[tostring(18458939117)]
	BerakUI.Main.Search.Shadow.Image = customAssets[tostring(5587865193)]
	BerakUI.Notifications.Template.Icon.Image = customAssets[tostring(77891951053543)]
	BerakUI.Notifications.Template.Shadow.Image = customAssets[tostring(3523728077)]
	BerakUI.Loading.Banner.Image = customAssets[tostring(111263549366178)]
...
if BerakUI.AbsoluteSize.X < minSize.X and BerakUI.AbsoluteSize.Y < minSize.Y then
...
local Main = BerakUI.Main
local MPrompt = BerakUI:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = BerakUI:FindFirstChild('Drag')
...
BerakUI.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/BerakUI/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = BerakUI.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when BerakUI is destroyed

local SelectedTheme = BerakUILibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = BerakUILibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	BerakUI.Main.BackgroundColor3 = SelectedTheme.Background
	BerakUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	BerakUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	BerakUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	BerakUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	BerakUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	BerakUI.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		BerakUI.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		BerakUI.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(BerakUI:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("BerakUI | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("BerakUI | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('BerakUI had an issue decoding the configuration file, please try delete the file and reopen BerakUI.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(BerakUILibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("BerakUI | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--BerakUILibrary:Notify({Title = "BerakUI Flags", Content = "BerakUI was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(BerakUILibrary.Flags) do
...
function BerakUILibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('BerakUI | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			BerakUILibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			BerakUILibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "BerakUIOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, BerakUIFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('BerakUI Settings', 0, true)

	if TabList['BerakUI Settings'] then
		TabList['BerakUI Settings'].LayoutOrder = 1000
	end

	if Elements['BerakUI Settings'] then
		Elements['BerakUI Settings'].LayoutOrder = 1000
	end
...
function BerakUILibrary:CreateWindow(Settings)
	if BerakUI:FindFirstChild('Loading') then
		if getgenv and not getgenv().BerakUICached then
			BerakUI.Enabled = true
			BerakUI.Loading.Visible = true

			task.wait(1.4)
			BerakUI.Loading.Visible = false
		end
	end

	if getgenv then getgenv().BerakUICached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				BerakUILibrary:Notify({Title = 'Build Mismatch', Content = 'BerakUI may encounter issues as you are running an incompatible interface version ('.. ((BerakUI:FindFirstChild('Build') and BerakUI.Build.Value) or 'No Build') ..').\n\nThis version of BerakUI is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "BerakUI"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "BerakUI Interface Suite" then
		LoadingFrame.Version.Text = "BerakUI UI"
	end
	...
	if not Settings.DisableBerakUIPrompts then
		task.spawn(function()
			while not BerakUIDestroyed do
				task.wait(math.random(180, 600))
				if BerakUIDestroyed then break end
				BerakUILibrary:Notify({
					Title = "BerakUI Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio and not secureMode then
		ensureFolder(BerakUIFolder.."/Discord Invites")

		if not callSafely(isfile, BerakUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, BerakUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"BerakUI RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(BerakUIFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("BerakUI | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BerakUI specific development.')
				end
			end
		end
		...
		if callSafely(isfile, BerakUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, BerakUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
		...
		if not Passthrough and secureMode then
			warn("BerakUI | Secure Mode: Key system requires a valid saved key. The key UI cannot be shown as it requires loading detectable assets.")
			BerakUI.Enabled = false
			return BerakUILibrary
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, BerakUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						BerakUILibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	if Settings.KeySystem then
		...
		if BerakUIDestroyed then return end
	end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called BerakUILibrary:Destroy()
				if BerakUIDestroyed then
					return
				end
				if not Success then
					...
					print("BerakUI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BerakUI specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= BerakUILibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					BerakUILibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			BerakUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("BerakUI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BerakUI specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					BerakUILibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("BerakUI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with BerakUI specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					BerakUILibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("BerakUI | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with BerakUI specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					BerakUILibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= BerakUILibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("BerakUI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BerakUI specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						BerakUILibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("BerakUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with BerakUI specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					BerakUILibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			BerakUILibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			BerakUILibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('BerakUI had an issue creating settings.') end
...
function BerakUILibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function BerakUILibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function BerakUILibrary:Destroy()
	BerakUIDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['BerakUI Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "BerakUIOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function BerakUILibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				BerakUILibrary:Notify({Title = "BerakUI Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			BerakUILibrary:Notify({Title = "BerakUI Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('BerakUI Configurations Error | '..tostring(result))
			BerakUILibrary:Notify({Title = "BerakUI Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = BerakUILibrary:CreateWindow({
	--	Name = "BerakUI Example Window",
	--	LoadingTitle = "BerakUI Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using BerakUI may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like BerakUI to get the key from
	...
	----BerakUILibrary:Notify({Title = "BerakUI Interface", Content = "Welcome to BerakUI. These - are the brand new notification design for BerakUI, with custom sizing and BerakUI calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(BerakUILibrary.Theme) do
...
task.delay(4, function()
	BerakUILibrary.LoadConfiguration()
